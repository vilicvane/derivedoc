...
- 钩子是在会话开始之后才安装的，那个已经开着的会话不会补加载它：实测本仓库主会话的几条用户
  ...
  会话才生效。
- 项目根可能不在当前目录上方（在父目录里工作、项目在它下面）。钩子先向上找 `.derivedoc`，
  找不到再查用户级注册表，而且只在**紧挨着恰好一个**项目时才认——否则在 `$HOME` 或
  `~/projects` 里干活会把消息记到某个深处的工作区去。认到的一律是项目根，文档目录再由
  `.derivedoc/config.json` 定位。
