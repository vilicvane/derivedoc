...
- `.derivedoc/commit-message` 只在它还描述当前这批改动时才算数：那批改动里有文件比它新，就
  当作过期——界面按当前改动草拟一句，并说明存着的那句描述的是更早的改动。一句写下来之后没人
  再动文档，它才继续当默认值。
- 界面里的「变更 N」按钮打开提交面板：文件列表、可点的 diff、commit message 输入框和提交
  按钮；message 为空时按改动文件自动草拟一句。
- 提交不要求先暂存：一篇都没暂存时，提交按钮会把两层文档的改动一起 add 再 commit；暂存过
  则只提交暂存的那几篇（想拆开提交就先暂存一部分）。
- 文档页的工具栏也有「暂存 / 取消暂存」：只处理当前这篇，暂存后主 diff（以暂存区为基准）
  自然变空；两者同时出现说明这篇既有已暂存内容、又有新的未暂存改动。
...
编辑器里的改动先落在草稿里（内存，不落盘）。草稿按文档存，切走再回来还在；「未保存」标记只在
草稿和磁盘不一样时亮——撤销到底、内容回到磁盘版本，就把草稿丢掉。不丢的话，切走之后没有比对
对象，标记会凭空亮着。草稿同时记住它基于的磁盘正文与 `revision`。

磁盘在别处动过（agent 写入、外部编辑）时草稿不丢也不自动覆盖：`loadDoc` 发现草稿基于的正文和
磁盘对不上就置 `incoming`，文档页显示「磁盘上出现了新版本」横幅；保存按草稿自己的 `revision`
提交，服务端 409 就保留草稿并提示，由人决定用哪一版。

TODO: 草稿与磁盘分叉之后怎么收敛还没定——现在是「用磁盘版本」或者手工把差异合回去。

编辑器每篇一个 model（`editor-models.ts`），切文档只是 `setModel`，撤销栈跟着文档各留各的；
正文真的被换掉时走整段替换的 `pushEditOperations`，不用 `setValue`——它内部会清掉 command
manager，整条撤销栈一起没。对比视图的正文那侧共用同一个 model，编辑与对比之间切换也不丢撤销。
